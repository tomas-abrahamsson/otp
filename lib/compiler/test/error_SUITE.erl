...
warnings_as_errors(Config) when is_list(Config) ->
    ...
    ?line [] = run(Config, Ts, write_beam),

    %% TODO: avoid constructing filename twice
...
run(Config, Tests) ->
    run(Config, Tests, dont_write_beam).

run(Config, Tests, WriteBeam) ->
    F = fun({N,P,Ws,E}, BadL) ->
                case catch run_test(Config, P, Ws, WriteBeam) of
                    E -> 
                        BadL;
                    Bad -> 
...
run2(Config, Tests) ->
    run2(Config, Tests, dont_write_beam).

run2(Config, Tests, WriteBeam) ->
    F = fun({N,P,Ws,E}, BadL) ->
                case catch filter(run_test(Config, P, Ws, WriteBeam)) of
                    E ->
                        BadL;
                    Bad ->
                        ?t:format("~nTest ~p failed. Expected~n  ~p~n"
...
run_test(Conf, Test0, Warnings, WriteBeam) ->
    ?line Test = ["-module(errors_test). ", Test0],
    ?line File = test_filename(Conf),
